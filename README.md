...
| `render/arena/` | The **public, judge-runnable arena** (`noleak-arena` web service): pick a victim model + toggle guard/invariant, launch the attack against a live Nebius agent, watch the real detectors decide LEAKED / DENIED / declined. Simulated Slack + agent loop, **real detectors imported from `plugins/`**, real Nebius model. Live + instant Replay, rate-limited. | Sep 8 |
| `tests/` | Network-free unit tests: guard 22, scorer 9, invariant 5, arena 11 (fake LLM + fake fetch) (`node --test tests/`, Node 18+). | Sep 5–8 |
